...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = "Будьласка оберіть розмір та матеріал картини";
...
const checkTextInputs = selector => {
  ...
  txtInputs.forEach(input => {
    input.addEventListener('keypress', function (e) {
      if (e.key.match(/[^а-щА-ЩЬьЮюЯяЇїІіЄєҐґ 0-9]/ig)) {
        e.preventDefault();
      }
    });
  });
...
const forms = () => {
  ...
  const message = {
    ...
    success: 'Дякую! Незабаром ми вам зателефонуємо',
  ...
  const clearInputs = () => {
    ...
    upload.forEach(item => {
      item.previousElementSibling.textContent = "Виберіть файл";
    });
  };
  upload.forEach(item => {
...
_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"];
